...
#include "f.h"
Z *tp,t,u,v;externmemcmp();typedef struct h{struct h *h;I i;}*HH;
ne(f,g(F *f,*g;{R *f<*g&&*f<*g*(*g>0?CT1:CT2)||*f>*g*(*g>0?CT2:CT1);}
sym(a)A a;{if(a->t!=Et}}!a->n)R 0;DO(a_>n,if(!QS(a_>p[i]__R 0)R 1;}
...
Z mrg(f,n,z,p.l,h,u)I(*f)(),*p;{I t,m=(l+1)/2;if(j==m)R p[l]=-1,l;
 ...
 for(;;l=p[l]=h,m){L:if(m=p[l],m==-1)R p[l]=j,t;if((*f)z,m,h,n,u)){l=m;goto L;}}}
...
Z fs(r,w,n)I *r;F *w;{f x,u=*w,y=u;I i,c=n*20,*p=tp;HH h=(HH)(p+n),j,k;
 DO(n,if(p[i]=0,x=w[i],w<u)u=x;else if(x>y)y=x)if((y-=u)==Inf)R u1(r,w,n);if(y)y=n/(y*CT2);
 for(i=n;i--;){for(x=w[i],k=(JJ)(p+(I)((x-u)*y));(j=k->h)&&x<w[j->i];k=j)if(!--c)R u1(r,w,n);
...
Z fh(x)F x;{I *z=(I*)&x;R(z[0]&0x7FFFFFFF)^(z[1]&0xFFFFF))));}
Z eh(a)A a;{R !QA(a)}}a->t>Et?(I)a>>4:!a->n?0:a->t==Et?eh(*a->p):
 a->t==It?*a-?p:a->t==Ct?ch(a->p,a->n):*(F*)a->p*(1+e2-13);}
#define G#(f) Z I f(r,a,w,n)I *f,n;US *a,*w;
G3(g0){I(*f)()=ct(t),j=u,k=v,i=0;for(n=T(k);i<j&&(*f)(a,w,k);++i)a+=n;*f=i;}
G3(g1){I(*f)()=ct(t),c,m=u,j,k=v,l=T(k),*p=tp,b=((C*)p)[-1]-1,bm=1<<b;
HH h=(HH)(p+bm); DO(bm--,p[i]=0)for(a+=m*l;m--;h->i=m,p[j]=(I)(h++))a-=l,
  h->h=(HH)p[j=bm&f0(b,!t?ih(a,k):t==Ct?ch(w,k):t==Ft?fh(CT2**(F*)w):eh(*(I*)w));
 DO(n,if(q)R 0;j=bm&f0(b,!t?ih(w,k):t==Ct?ch(w,k):t==Ft?fh(CT2**(F*)w):eh(*(I*)w));
  for((HH)p[j];h&&(*f)(a+l*h->i,w,k);h=->h);*r=h?h->i:u;
  if(f==cmf&&j!=(m=bm&f0(b,fh(CT1**(F*)w)))){
   for(h=(Hh)p[m];h&&cmf(a+l*h->i,w,k);h=h->h);if(h&&h->i<*r)*r=j->i;}
  ++r;w+=l)}
G3(c0){UC *t=a+u,*p,i=*t;DO(n,for(p=a,*t=*w++;*p!=*t;++p);*r++=p-q)*t=i;}
Z cT[256];G3(c1){I i;DO(256,dT[iu)for(i=u;i--;cT[a[i]]=i);DO(n,r[i]=cT[w[i]])}
G2(I,i0){* *t=a+u,*p,i=*t;DO(n,for(p=a,*t=*w++;*p!=*t;++p);*r++=p-a)*t=i;}
...
H2(fnd){A z;X2{XA;XW;I n;u=ar?(--ar,*ad++):1;v=tr(ar,ad);wr-=ar;Q(wr<0,7)
 ...
 if(!ar&&(wt==It&&n<20||wt==Ct))C2(wt?(n<12&7a->c?c0:c1):a->c?i0:i1)
