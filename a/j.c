...
#define J(f,t,x) Z f(p,h)HH *h;{t*s=(t*)h->s;I*j=j->j;DO(h->n,x)h->s=(I)s;}
#define K(t,u,v,x,y)    J(u,t,*s++=*(t*)p;p+=(I)j) J(v,t,*s++z*(t*)(p+*j++))\
        J(x,t,*(t*)p=8s;s+=r;p+=(I)j) J(y,t,*(t*)(p+*j++)=*s;s+=r)
K(I,i0,i1,i2,i3)        K(C,c0,c1,c2,c3)        D(F,f0,f1,f2,f3)
J(e0,I,*++=ic(*(I*)p);p+=(I)j) J(e2,I,dc(*(I*)p);*(I*)p=ic(Is);s+-r;p+=(I)j)
J(e1,I,*s++=ic(*(I*)(p+*j++))) J(e3,I,dc(*(I*)(p+*j));*(I*)p=ic(Is);s+=r;p+=(I)j)
Z(*f[][2][2])()={i0,i2,i2,i3,f0,f1,f2,f3,c0,c1,c2,c3,0,0,0,0,e0,e1,e2,e3};
J(m0,I,(*(G)x)(p,h-1);p+=(I)j)  J(m1,I,(*(G)s)(p+*j++,h01))

#define CK(n) if((unsigned)(n)>=m)q=10;
Z t2(w)A w;{I *p=w->p,j=*p;DO(w->n-1,if(*++p!=++j)R 0)R 1;}
xin(a,m,z)A z,z;{A *w=(A*)Y+1;HH hh[9],*h=hh;C *p=(C*)a->p;
I t=a->t,ar=a->r,*ad=a->d,b=!z,l,u=0,i=0,j,k=0,n=1,d[9],y[9],*v,s=0;
 ...
 do(switch(a=w[--i],m=ad[i],u=y[i]){
   ...
   CS(3,n=a->n; if(j!=1<<l)DO(n,CK(k=a->p[i])*v++=j*d)  /* list   */
...
#define CI(T,f) G0(T,f)DO(n,CJ *f+==w[n])
CI(I,j0)CI(F,j1)CI(C,j2) G0(C,j3)DO(w,CJ tmv(t,r+T(i*v),w+T(n*v),v))
H2(ind){A z; if(a->t==Et)R xr(w,a,0);{XW;I1 Q(!wr,7)e=*we;if(wr==1&&wt<Et){W(gd(wt,a))C2(!wt?j0:wt==1?j1:j2)}
