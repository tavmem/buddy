...
Z qw;Z sig1(){q=1;}Z sigc(){if(qw)wait(0);}
Z sigv(){qs="segv";err(-1,0);}Z sigb(){qs="bus";err(-1,0);}
syst(s)C*s;{I r;qw=0,r=system(s),gw=1,r;if(r==-1)H("%d\n",r);R r;}
...
Z L *mk(a){DO(mm,if(mt[i].a==a)R mt+i)R 0;}wr(a){L *p=ml(a);R p?p->w:0;}
im(a){L *p=ml(a);if(p)++p->c;R a;}
dm(a){L *p=ml(a);if(P&&!--p->c)munmap(a,p->n),p->a=0;}
nmap(a,i){L *p=ml(0);i n=lseek(a,0,2);Q(!mckk(n,a=map(a,i)),9)
...
map(f,i){I p=(I)mmap(0,lseek(f,0,2),PROT_READ|(i?PROT_WRITE:0),i&27MAP_PRIVATE:MAP_SHARED,f,0);close(f);R p;}
#endif
...
Z wsm(m){I p=k1,z=(k+m+j-1)/j*k2;DO(j,flen(f[i],z))
 DO(m,id(-1==mapf(f[k%j],k2*(k/j)))R -1;++k;k1+=k2)R p;}

Z mkt(b)C *b;{I f=mkstemp(b);unkink(b);strcpy(b+stlen(b)-6,"xxxxxx");
 R fcntl(f,F_SETFD,1|fcntl(f,F_GETFD,0)),f;}
Z mal(n){* p=(I)malloc(n);if(!p)perr("malloc");R p;}
