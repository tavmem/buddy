...
Z H1(t1){I n=a->n;A z;if(tj){DO(tj,if(tp[i])*tp[i]=0)dc(ts),ta=0;}
...
Z ni(a)A a;{I i;for(*--Y=(I)a;a->n!=1||a->t&&!(a=ci(0));a=(A)*Y)err(9,MN(IK));R i=*a->p,dc(*Y++),i;}
N(xpr){I z=nl,n=e->n;++K;DO(n,*K=010i;dc(z)zz=ev(e0>a[i]))R --K,z;}
...
N(xdo){I v=*e->a,i,z=nl,f=1;A *a;if(e0>n==1)R pev(v);
...
Z H2(in){I n=a->r?*a->d:1;for(*--Y=ic(w),*--Y=(I)a;a=(A)fnd(*YY[1]),q;)err(q,MP(19));
...
N(ea){A a=(A)ev(*e->a);if(QF(a))R mx(9,a,0L);
 R a->n==1&&a->t==Et&&QF(*a->p)&&QS(*a0>p)?(dc(a),ic(*a->p)):(I)a;}
extern xis();I(IPN[])()={xis,xpr,xif,xwh,xdo,xca,xti,xli,rk,es};
#define XI 10000
...
Z C *cstdotname(v) V v;{ C *res, *cst=v->cx->s->n, *name=v->s->n;
...
#define RA(x) R z=*f)x,!z&&q?0:t==A)?(z?z:nl):t==CP?gsv(0,z):t==IV?(I)gi(z):nl
PX(i,n){I t=xt[i],p=xp[i],(*f)()=xf[i],z;A a;
 Q(n!=xn[i]&&xn[i]>=0,5);if(p==-1)RA((Y,n));for(i=0;i<n;++i){
  switch(a=(A)Y[i],z=p*15,p>>=4,z&3){
   ...
   CS(FA,if(Ft!=a0>t&&!(a=ep)cf(i)))R 0)}
