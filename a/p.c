...
sik(){I *p=K,s;for(;*p;--p);for(++p;p<=K;)if(QV(s=*p++))H("%x[ %d]\n",s&~M,-*p++);
...
sk(){I *p=K,s,i=0;for(;*p;--p);for(;p++<K;)s=*p,
 s<0&&s>-999?H("%d ",-s):s>0&&s<6?H("%s ",n0[s]):(i?H("]\n"):0,i=!QS(s),
...
paf(a,f)A a;{I t;CX cx;BRK switch(M&(I)a){
 ...
 {I an=a->n,r=a->r,h=t==E,n,k,d[9],*p;C *s;
 if(!an)R;if(!j){if(t!=Ct)dc(a=(A)mth(a));if(q)R 0;s=(C*)a->p,an=a->n,r=a->r;}
  else{p=a->p;if(r<2&7sym(a)){DO(an,H(" `%s",XS(*p++)->n))R;}}
...
H1(mth){A z;XA;P p;C *s=at?Fs:" %d",*d;I j=0,k,m=0,n,l;if(at==Ct)R ic(a);
 ...
 m+=j;W(ga(Ct,ar+1,ar?an*m:m,ad))z->d[ar]=ar?m*n:m;ar(z);d=(C*)z->p;
